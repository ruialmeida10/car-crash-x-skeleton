...
abstract public class Car {
    ...
    private boolean rebooked = false;
    ...
    public void checkCrashed(int i, Car[] cars){
        ...
        for (int j = 0; j < cars[cars.length - 1].toString().length() ; j++){
            if (cars[cars.length - 1].getPos().getCol() + j  == cars[i].getPos().getCol() && cars[cars.length - 1].getPos().getRow() == cars[i].getPos().getRow()) {
                if (!rebooked)
                    cars[i].setCrashed(true);
                return;
            }
